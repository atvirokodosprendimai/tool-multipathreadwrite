...
- [x] The two Proposed-state `adr-lint` advice lines are resolved by T1 and T2 landing: `Enforced-by` now names a test that exists and `Governs: internal/mcp/**` now matches tracked files. Verified 2026-09-03 — neither is emitted on this tree.
- [ ] `adr-lint` still emits **twelve** advice lines, and an earlier draft of this list claimed they were gone. They are not, and they are not the three it named either — counted rather than recalled: three compare Acceptance-fence segments against `Rests-on:` names (10, 10 and 8 segments against 3, 3 and 2 names); one observes that T1's 196 ms exit-1 row is too short to have run the fence, which is correct and is the TDD-red row, recorded before `internal/mcp` compiled; and **eight** say a `Rests-on:` name has no `mutant killed` row bound to it. That last group is the real one. Five mutants are recorded across the three tasks and each is bound to a fence, but the linter wants one per declared mechanism, and `the absence of a new go.mod requirement`, `the identical apply.Result`, `the in-process serialization`, `the host config block` and `the statement that the CLI path is unchanged` have none. Worth closing one at a time rather than in a batch, since a mutant chosen to satisfy a counter is the failure this pipeline exists to refuse
