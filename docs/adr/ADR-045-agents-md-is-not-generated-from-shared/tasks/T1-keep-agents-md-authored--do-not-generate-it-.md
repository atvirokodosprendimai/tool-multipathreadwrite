# Task ADR-045-T1: Keep AGENTS.md authored. Do not generate it.
...
Keep AGENTS.md authored. Do not generate it.

## Affected Files

| File | Change | Why |
|------|--------|-----|
| `internal/guide/guide.go` | none — record only | Shared() stays the pamphlet |
...
```bash
set -o pipefail
grep -F -q '**Status:** Accepted' docs/adr/ADR-045-agents-md-is-not-generated-from-shared.md \
  && grep -E -q 'record only|measurement only|measure first' docs/adr/ADR-045-agents-md-is-not-generated-from-shared.md
```
...
|-----------|------|----------|--------|-------|
| `Acceptance fence` | `docs/adr/ADR-045-agents-md-is-not-generated-from-shared.md` | Status is Accepted and the Decision names record-only or measure-first | — | S1 |
