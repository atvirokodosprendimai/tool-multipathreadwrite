...
```bash
set -o pipefail
go test ./internal/rooted/ ./internal/state/ ./internal/read/ ./internal/mcp/ ./cmd/mrw/ -count=1 -timeout 240s -run 'TestAPathInsideMrwsStateIsRefused|TestBaseDoesNotCreateTheStateDirectory|TestTheWalkServesNothingFromMrwsState|TestTheAckStoreCannotBeReadOverMCP|TestAPlanCannotEditTheLedger|TestAstGrepDropsAHitInsideMrwsState' -v 2>&1 | tee /tmp/adr077-T1.out \
  && missing=$(for t in TestAPathInsideMrwsStateIsRefused TestBaseDoesNotCreateTheStateDirectory TestTheWalkServesNothingFromMrwsState TestTheAckStoreCannotBeReadOverMCP TestAPlanCannotEditTheLedger TestAstGrepDropsAHitInsideMrwsState; do grep -qE "^--- PASS: $t \(" /tmp/adr077-T1.out || echo "$t"; done) \
  && [ -z "$missing" ] \
