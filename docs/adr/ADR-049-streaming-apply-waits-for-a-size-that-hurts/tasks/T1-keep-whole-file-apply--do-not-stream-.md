# Task ADR-049-T1: Keep whole-file apply. Do not stream.
...
Keep whole-file apply. Do not stream.

## Affected Files

| File | Change | Why |
|------|--------|-----|
| `internal/apply/apply.go` | none — record only | apply still reads the file |
...
```bash
set -o pipefail
grep -F -q '**Status:** Accepted' docs/adr/ADR-049-streaming-apply-waits-for-a-size-that-hurts.md \
  && grep -E -q 'record only|measurement only|measure first' docs/adr/ADR-049-streaming-apply-waits-for-a-size-that-hurts.md
```
...
|-----------|------|----------|--------|-------|
| `Acceptance fence` | `docs/adr/ADR-049-streaming-apply-waits-for-a-size-that-hurts.md` | Status is Accepted and the Decision names record-only or measure-first | — | S1 |
