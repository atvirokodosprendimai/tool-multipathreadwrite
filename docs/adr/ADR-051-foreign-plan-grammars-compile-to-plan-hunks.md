...
**Governs:** `cmd/mrw/main.go`, `scripts/contract.sh`, `internal/ingest/applypatch.go`, `internal/ingest/searchreplace.go`, `internal/mcp/mcp.go`, `internal/mcp/tools.go`, `internal/mcp/instructions.go`
...
**Served-path change:** `mrw write --format=apply_patch [PLAN|-]` compiles a Codex `apply_patch` document into `@@` hunks, then `plan.Parse` and `apply.Apply` run unchanged. `--format=search_replace` compiles an Aider SEARCH/REPLACE document the same way. Default `write` is still the native plan. A git patch is not an `apply_patch`. Over MCP the same compiles are `mrw_write` optional `format` (`plan` default, `apply_patch`, `search_replace`); `git` is refuse. No third tool.
...
Four tracked files on the write path (1+1+1+1). The compiler package is new and is named by the tasks. Members left out: git unified diffs (permanent: they are not this grammar). MCP `format` on existing `mrw_write` shipped 2026-09-12 (F-27). SEARCH/REPLACE shipped 2026-09-12 as `--format=search_replace` (F-26).
...
|---------|--------|----------|-------------|
| `mrw write --format` | `plan` (default), `apply_patch`, or `search_replace` | `cmd/mrw/main.go` `writeCmd` | CLI callers; contract §82, §84 |
...
| `mrw_write.format` | optional; `plan` default, `apply_patch`, or `search_replace`; `git` refuse | `internal/mcp/mcp.go` `tools()` / `writeTool` | MCP hosts; contract §83, §84 |
...
| `internal/ingest.CompileSearchReplace` | new | T4 | `writeCmd`; MCP `writeTool` |
...
| `mrw_write.format` (T3) | T3 | T4 | No — enum grows |
