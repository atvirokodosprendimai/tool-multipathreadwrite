...
6. [S6] ⚠ **A served-line control must assert the REFUSAL, not only that the line appears.** The `delete` control added in S5 grepped the output for `public line` and checked no exit code, so it passed against a binary that had stopped checking anchors: `delete`'s SUCCESS receipt prints the removed line, ADR-008 having made it say what it removed. The fourth Codex review of PR #128 found it, and `insert-after`'s control had the same shape — surviving only because its receipt prints no content, which is an accident of receipt shape and not a property of the check. Both now assert exit 1 and match the anchor message. Bound to a mutant rather than to prose: with the anchor comparison disabled for `replace` and `delete`, the section goes red on `an anchored delete on a SERVED line is still anchor-checked (exit 0, want 1)`. [proof: mutation]
7. [S7] ⚠ **One fresh fixture per case, or the section tests whichever case ran first.** All eight assertions shared one file and one ledger. A case that wrongly SUCCEEDS writes the file and records the ledger WHOLE, so every later case becomes a different test than its name says — an unserved line that is no longer unserved, a sentinel that is no longer there. That is not hypothetical: the S6 mutant run printed `from "X" to "X"` where `public line` was meant, because an earlier hunk had already rewritten it. Found by the fifth Codex review of PR #128. The section now calls `anchored_fixture` before each case, and the eight assertions became sixteen across four ops — including the ledger's own message asserted PRESENT beside each sentinel-ABSENT check, since an absence assertion alone is satisfied by empty output. The isolation is what makes the mutant discriminate: with the anchor disabled on the `replace`/`delete` site only, those two controls go red and the two insertion controls stay green. [proof: mutation]
