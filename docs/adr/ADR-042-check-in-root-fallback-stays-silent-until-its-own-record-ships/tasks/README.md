...
Implementation tasks for ADR-042: Check in-root fallback stays silent until its own record ships. See the parent ADR for the decision.
...
|----|-------|--------|--------|------------|
| T1 | Keep the record. Do not change the fallback. | pending | — | `grep` the Accepted record |
