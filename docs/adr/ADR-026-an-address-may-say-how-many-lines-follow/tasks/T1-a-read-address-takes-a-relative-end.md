...
| File | Change | Why |
|------|--------|-----|
| `internal/read/read.go` | edit | Four sites. (a) `splitRanges` gains a `,+` lookahead beside the `,/` one it already has, so the suffix stays attached to the range it belongs to instead of starting a new one. (b) `parseRange` cuts a trailing `,+N` off before it parses `A`, and stores `N` in the new field. (c) `Range` gains `RelEnd int` — 0 means "no relative end", which is safe because `+0` is refused. (d) `resolve` applies it where the file length is already known, which is where `$` is resolved and therefore where the clamp is free. |
...
3. [S3] Teach `splitRanges` to keep `,+` attached, beside the existing `,/` lookahead, and say in the comment that these are now two cases of one rule: a comma followed by `+` or `/` continues the range, and every other comma separates. [proof: mutation]
...
| 2 — something selects it | `splitRanges` and `parseRange` are on the only path `mrw read` and `mrw_read` take to an address; the S3 mutation removes the `,+` lookahead so the suffix becomes a second address again, and the fence must go red |
...
- `splitRanges` is scanning bytes, and a `,+` inside a regex must not be caught by the new lookahead. The existing loop already consumes a `/…/` wholesale before it looks at the next byte, so a `,+` inside a pattern is never seen by the comma case — asserted by keeping `TestRegexpRange` and `TestParseSpec` in the fence.
