...
**Proof map:** v1
**Rests-on:** `a second holder waits`, `racing updates keep every entry`, `racing records keep every count`, `a reader never sees a half-saved file`, `mrw seen reads under the lock`, `a contract row drives the binary`, `the packages vet for Windows`, `the other engine packages are unchanged`, `go.mod declares one requirement`
...
2. [S2] Implement; GREEN. [proof: mutation]
   Mutants: `Hold` takes no lock; `Update` takes no lock; `locked` takes no lock; `mrw seen` back on `seen.Load`.
