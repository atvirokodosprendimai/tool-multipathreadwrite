...
| Every CLI check run leaves a `mrw-check-*.log` in the system temp directory, and nothing removes it | **open** — found 2026-09-24 while fixing contract.sh's leak (3,103 on one macOS machine; `internal/check/check.go:203` `os.CreateTemp("", …)`). The log is what `full output:` points at, so deleting it on success, or rotating a bounded set, is the decision | — |
