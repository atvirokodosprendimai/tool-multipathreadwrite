...
4. [S4] Implement the pending store under `internal/state`'s directory (ADR-004: nothing in the working tree), keyed by checkpoint, holding path, sha and span. Cap the entries per root and drop the oldest beyond it — `TestThePendingStoreIsBounded` is its oracle, added after the sixth review of PR #132 found the bound claimed as `[proof: mutation]` with nothing observing it. [proof: acceptance]
...
go test ./internal/mcp/ -count=1 -v \
  -run 'TestACheckpointCoversTheSpanItBrackets|TestAPendingRecordReachesNoLedger|TestThePendingStoreIsBounded' 2>&1 | tee /tmp/adr031-t1.out \
  ...
  && grep -q '^--- PASS: TestThePendingStoreIsBounded' /tmp/adr031-t1.out \
...
- 2026-09-07 · fc95241* · mutant killed · exit 1 · `internal/mcp/ack.go` · eviction stops, so a caller that never acknowledges grows the pending store without limit — the bound T1 declared [proof: mutation] while nothing observed it, until TestThePendingStoreIsBounded was added (sixth review of PR #132) · acceptance-sha256:661d553052712c9edccf3d5edde2eb93caf6a916c563fb6b5bd1f3280a3740bf · covers:a pending record reaching no ledger until it is promoted
