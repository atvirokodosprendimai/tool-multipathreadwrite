# Task ADR-041-T1: Keep the record. Do not write a version-negotiation protocol.
...
Keep the record. Do not write a version-negotiation protocol.

## Affected Files

| File | Change | Why |
|------|--------|-----|
| `docs/adr/ADR-041-path-binary-and-skill-version-skew-is-named-not-healed.md` | none — record only | the decision |
...
```bash
set -o pipefail
grep -F -q '**Status:** Accepted' docs/adr/ADR-041-path-binary-and-skill-version-skew-is-named-not-healed.md \
  && grep -E -q 'record only|measurement only|measure first' docs/adr/ADR-041-path-binary-and-skill-version-skew-is-named-not-healed.md
```
...
|-----------|------|----------|--------|-------|
| `Acceptance fence` | `docs/adr/ADR-041-path-binary-and-skill-version-skew-is-named-not-healed.md` | Status is Accepted and the Decision names record-only or measure-first | — | S1 |
