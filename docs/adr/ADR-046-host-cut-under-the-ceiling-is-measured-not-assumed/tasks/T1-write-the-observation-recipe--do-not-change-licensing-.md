# Task ADR-046-T1: Write the observation recipe. Do not change licensing.
...
Write the observation recipe. Do not change licensing.

## Affected Files

| File | Change | Why |
|------|--------|-----|
| `internal/mcp/tools.go` | none — measure only | fitting serve stays |
...
```bash
set -o pipefail
grep -F -q '**Status:** Accepted' docs/adr/ADR-046-host-cut-under-the-ceiling-is-measured-not-assumed.md \
  && grep -E -q 'record only|measurement only|measure first' docs/adr/ADR-046-host-cut-under-the-ceiling-is-measured-not-assumed.md
```
...
|-----------|------|----------|--------|-------|
| `Acceptance fence` | `docs/adr/ADR-046-host-cut-under-the-ceiling-is-measured-not-assumed.md` | Status is Accepted and the Decision names record-only or measure-first | — | S1 |
