...
// observation of "x " loaded as "x" and licensed a file nobody had read. A
// trailing "\r" is the same defect one layer down: bufio.ScanLines drops it.
func TestALedgerPathKeepsItsSurroundingSpaces(t *testing.T) {
	root := t.TempDir()
	t.Setenv("XDG_STATE_HOME", t.TempDir())
	paths := []string{"x ", " y", "a  b", "z\r"}
	...
	for _, trimmed := range []string{"x", "y", "z"} {
