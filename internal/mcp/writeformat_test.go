...
func TestWriteDeclaresFormatOnTheExistingTool(t *testing.T) {
	...
	if !strings.Contains(got, "search_replace") {
		t.Error("instructions do not name search_replace")
	}
	if !strings.Contains(desc, "search_replace") {
		t.Errorf("format does not name search_replace: %v", f)
	}
...
func TestASearchReplaceBlobOnWritePlanIsABadNativePlan(t *testing.T) {
	root, _ := checkout(t, "a.go", demoFile)
	res := call(t, root, "mrw_write", map[string]any{"plan": twoHunkSearchReplace})
	if res["isError"] != true {
		t.Fatalf("a SEARCH/REPLACE blob without format was not a parse refusal: %v", res)
	}
	b, err := os.ReadFile(filepath.Join(root, "a.go"))
	if err != nil {
		t.Fatal(err)
