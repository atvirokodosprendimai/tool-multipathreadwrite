...
**Status: stable at v1.14.0 (2026-09-12), the tag cut from `085ad95`.**
v1.14.0 is apply_patch ingest plus the why: `--format=apply_patch` and
`--format=search_replace` compile to a native plan; a failed hunk writes
nothing because a write that changed nothing is invisible. `guide.Shared()`
stays the five ADR-037 sentences. The handshake bound stays 4096. Sequential
apply_patch is the leak, not the feature. §82–85 drive the shipped binary:
unread still writes nothing, MCP `format` is a flag not a third tool,
search_replace compiles, and the binary prints the why.
...
and is not quoted flat for that reason: **802 pass on darwin/arm64, and the Linux CI run for the tag
reports 799 passing, one skipped and none failing.** The gap is exactly the case-folding family —
...
skip line in run 34712721240 says *"a case-sensitive filesystem here: the two-spelling half did not
...
(`scripts/break-campaign.sh`, its run in `docs/break/campaign-v1.14.0.txt`) against that same tagged
tree found no silent wrong write and every refusal in it names its reason; every probe's name and
exit code is identical to the v1.13.0 run against `d7e39bd`, the v1.12.0 run against `c4384f3`, the
v1.11.0 run against `69e1d1f`, the v1.10.0 run against `c60b20d`, the v1.9.0 run against `2363abb`, the
v1.8.0 run against `fe49ef5`, the v1.7.0 run against `8846325`, the v1.6.0 run against `0fffa77`, the
v1.5.0 run against `07bc664`, the v1.4.0 run against `bd73ee0`, the v1.3.0 run against `3434c35`, the
v1.2.0 run against `03feb92` and the v1.1.0 run against `d6c62e7`.
...
what v1.14.0 changes, and contract §82–85 do.
